    ...
    public class OsobaUredjajController : BaseController<OsobaUredjaj, OsobaUredjajDTO>
    {
        ...
        public IActionResult MenjanjeEntiteta(long id, OsobaUredjajDTO novi)
        {
            try
            {
                ...
                {
                    return BadRequest("Nema sa tim ID nista.");
                }
                ...
                //ako se uredjaj vec koristi 
                var akoSeKorsiti = _repository.DajSveEntitete()
                    .Where(vec => vec.Uredjaj.Naziv == novi.Uredjaj.Naziv && vec.KrajKoriscenja > DateTime.Now)
                    .FirstOrDefault();
